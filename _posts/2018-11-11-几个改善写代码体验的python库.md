...
tqdm有三种十分通用的方法：
1. Iterable-based：将任意可迭代对象通过tqdm()封装即可。
```python
text = ""
...
for i in tqdm(range(100)):
    print(i)
```
2. Manual: 手动控制可以通过一个with语句来完成:
```python
with tqdm(total=100) as pbar:
    for i in range(10)
        pbar.update(10)
```
3.Module: tqdm在脚本或者命令行中的应用是十分方便的，只用在管道中插入tqdm(或者python -m tqdm),它会将stdin传递给stdout的同时将进度打印在stderr。
## ARGPARSE
...
## END
今天刚好没事就随便写了写，我觉得我以后应该把文章的重心转移到一些深度学习算法和一些更深的思考上去。